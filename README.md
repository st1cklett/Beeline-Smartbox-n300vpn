...
Я сделал этот проект для старого **Beeline SmartBox N300**. Он запускает лёгкий VLESS-клиент на роутере, а все рабочие файлы хранит на USB-флешке. Внутренняя flash-память, ядро и Wi‑Fi-драйвер не меняются.

Целевая платформа: **Realtek RTL8197D/Lexra, 660 МГц, 64 МБ RAM, OpenWrt Barrier Breaker 14.07**. На других моделях бинарник не гарантируется. Картинка выше — иллюстрация проекта, не официальное изображение Beeline.
...
Нужны роутер с OpenWrt, USB-флешка ext4, Ethernet-кабель, компьютер и ваш root-пароль SSH. На время установки подключите только один LAN-кабель и выключите VPN на компьютере.
...
Слабый Wi‑Fi на старом OpenWrt — отдельное ограничение штатного драйвера. Я не рекомендую менять мощность, ядро или драйвер случайными пакетами. Установку и первые тесты проводите через LAN. Не выдёргивайте флешку во время обновления списков и не запускайте несколько проверок одновременно.
